...
LIB_API float validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, float *loss);
...
LIB_API unsigned short early_stopping_system(float valid_loss);
